...
// FINAL FIX: cfg is read inside useEffect (browser-only), not at component top level (SSR)
...
function getReply(input: string): { a: string; o?: string[] } {
  ...
  return { a: "I'm not sure, but our advisors can help! 😊\n\n📱 Call: +91 7009933289\n💬 WhatsApp us below", o: ['View Courses', 'Talk to Advisor', 'Contact Us'] }
}

export default function FloatingChatbot() {
  // ⚠️ KEY FIX: cfg is initialized with defaults and set in useEffect (browser-only)
  // At SSR time (server render), window doesn't exist — so we must NOT read it at top level
  const [cfg, setCfg] = useState<ChatConfig>(DEFAULT_CONFIG)
  ...
  useEffect(() => {
    // Now we are in the browser — read window safely
    const windowConfig = (window as any).__CHATBOT_CONFIG
    if (windowConfig) {
      setCfg({ ...DEFAULT_CONFIG, ...windowConfig })
    }
  ...
  return (
    <>
        ...
        <div style={{ position:'fixed', bottom:'88px', right:'20px', width:'320px', height:'490px', background:'#fff', borderRadius:'16px', boxShadow:'0 12px 40px rgba(0,0,0,0.18)', display:'flex', flexDirection:'column', zIndex:99999, overflow:'hidden', border:'1px solid #e2e8f0', fontFamily:'Inter,sans-serif' }}>
          <div style={{ background:pc, padding:'13px 15px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'9px' }}>
              <div style={{ width:'34px', height:'34px', borderRadius:'50%', background:cfg.accent, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', flexShrink:0 }}>{cfg.bot_avatar}</div>
              <div>
                ...
                <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.7)', display:'flex', alignItems:'center', gap:'4px' }}>
                  <span style={{ width:'5px', height:'5px', borderRadius:'50%', background:'#4ade80', display:'inline-block' }} /> Online now
                </div>
              </div>
            </div>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'12px', display:'flex', flexDirection:'column', gap:'9px', background:'#f8fafc' }}>
              ...
              <div key={m.id} style={{ display:'flex', flexDirection:'column', alignItems: m.from === 'user' ? 'flex-end' : 'flex-start', gap:'5px' }}>
                ...
                {m.options && m.from === 'bot' && (
                  <div style={{ display:'flex', flexWrap:'wrap', gap:'5px', maxWidth:'90%' }}>
                    {m.options.map(o => (
                      <button key={o} onClick={() => send(o)} style={{ padding:'4px 10px', background:'#fff', color:pc, border:`1.5px solid ${pc}`, borderRadius:'20px', fontSize:'11px', fontWeight:'600', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>{o}</button>
                    ))}
                  </div>
                )}
              </div>
            ...
            {typing && (
              <div style={{ display:'flex', gap:'4px', padding:'10px 12px', background:'#fff', border:'1px solid #e2e8f0', borderRadius:'13px 13px 13px 3px', width:'fit-content' }}>
                {[0,1,2].map(i => <span key={i} style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#94a3b8', display:'inline-block', animation:`cb-bounce 1.2s ease-in-out ${i*0.2}s infinite` }} />)}
              </div>
            )}
            {showLead && !leadOk && (
              <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:'12px', padding:'12px' }}>
                ...
                <button onClick={submitLead} style={{ width:'100%', padding:'8px', background:pc, color:'#fff', border:'none', borderRadius:'7px', fontWeight:'700', fontSize:'12px', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>Request Callback 📞</button>
              </div>
            )}
            <div ref={endRef} />
          </div>
          ...
          <div style={{ padding:'10px', borderTop:'1px solid #e2e8f0', background:'#fff', flexShrink:0 }}>
            <div style={{ display:'flex', gap:'7px', marginBottom:'7px' }}>
              ...
              <button onClick={() => send()} style={{ width:'33px', height:'33px', borderRadius:'50%', background:pc, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:'14px', flexShrink:0 }}>➤</button>
            </div>
      ...
      <button onClick={openChat} aria-label="Open chat" style={{ position:'fixed', bottom:'20px', right:'20px', width:'56px', height:'56px', borderRadius:'50%', background:pc, border:'none', cursor:'pointer', boxShadow:'0 4px 16px rgba(28,61,122,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:99998 }}>
        ...
        {!open && unread > 0 && (
          <div style={{ position:'absolute', top:'-2px', right:'-2px', width:'17px', height:'17px', borderRadius:'50%', background:'#ef4444', border:'2px solid #fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'9px', fontWeight:'800', color:'#fff' }}>{unread}</div>
        )}
      </button>
